...
export class Viewer extends EventDispatcher {
	...
	/**
	 *  Retrieves in one list all the items related to the viewer, including
	 * 
	 *  Measurements
	 *  Clipping Volumes
	 *  Annotations
	 ...
	 *  Images3D
	 *  
	 *  Items from loaders like OBJLoader or custom 
	 *  
	 * In any case, each category conforms a scene and each includes custom lighting. A limitation
	 * occurs for loaders which share the same scene, so they must be carefully managed.
	 * 
	 * 
	 * 
	  */
	enumerateAssets() {
		console.log('Enumerated known assets and providing common api')

		let currentId = 0;
		let assets = []
		//starting with known locations
		// pointclouds

		let pointcloudsArray = viewer.scene.pointclouds
		for (let i = 0; i < pointcloudsArray.length; i++) {
			let pc = pointcloudsArray[i];
			...
			assets.push(pc_asset)
			currentId++;

		}

		let annotationsArray = viewer.scene.annotations.children

		for (let i = 0; i < annotationsArray.length; i++) {
			let annotation = annotationsArray[i];

			let annotation_asset = {
				id: currentId,
				type: 'ANNOTATION',
