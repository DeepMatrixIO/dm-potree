...
export class ProfileWindow extends EventDispatcher {
	constructor(viewer) {
		...
		this.projectedBox = new THREE.Box3(); // <----------------------- required
		this.pointclouds = new Map(); // <----------------------- required
		this.numPoints = 0; // <----------------------- required but nor strictly usefull
		this.lastAddPointsTimestamp = undefined;    // NO IDEA

		this.mouse = new THREE.Vector2(0, 0);  // UI required for position of mouse
	...
	//##################################
		...
		//##################################
		this.initTHREE();  // <----------------------- required. As a THREE.js scene, requires a new rendered ,, camera, scene, etc.
		this.initSVG();   // scales, grid, 
		this.initListeners(); // 

		this.pRenderer = new Renderer(this.renderer);// setting rendered
	...
	// Setups renderer , dom elements, dimensions, 
	//also sets renderer context to draw
	//to support profile views, an ortographic camera is used
	initTHREE() {
