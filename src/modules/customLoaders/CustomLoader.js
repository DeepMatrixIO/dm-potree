...
export class CustomLoader {
	...
	static load(url, callback) {
		try {
			let pco = new PointCloudOctreeGeometry();//only a container 
			pco.url = url;
			...
			xhr.onreadystatechange = function () {
				if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
					...
					pco.projection = fMno.projection;//forcefully added
					pco.boundingBox = boundingBox;//already exists
					pco.tightBoundingBox = tightBoundingBox;//forcefully added
					pco.boundingSphere = boundingBox.getBoundingSphere(new THREE.Sphere());//forcefully added
					pco.tightBoundingSphere = tightBoundingBox.getBoundingSphere(new THREE.Sphere());//forcefully added
					pco.offset = offset;//forcefully added
					...
					//////////////////// creating the geometric support to compute visibility


					let nodes = {};//nodes dictionary
					//eacho node is a PointCLoudOctreeGeometryNode
					...
					{ // load root
						...
						pco.root = root;
						pco.root.load();//most important method.
						nodes[name] = root;
					}
