...
export class Annotation extends EventDispatcher {
	constructor(args = {}) {
		...
		//	textarea.innerText=this._title
		...
		//adding behaviour to modify title
		this.elTitlebar.on('dblclick', (event) => {
			//https://en.js.cx/task/editable-div/solution/
			console.log('dblclick')

			let editable = document.createElement('textarea',{className:'annotation-label-edit', class:'annotation-label-edit'
			})
			...
			event.currentTarget.replaceChildren(editable);

			//event.currentTarget.append(editable)
			editable.focus();
			...
			editable.onkeydown = function (innerevent) {
				if (innerevent.key == 'Enter') {
					
					this.blur();
					//innerevent.target.parentElement.replaceChildren(document.createElement('span'))
