...
var OBJLoader = (function () {
	...
	function ParserState() {
		...
		var state = {
			...
			//simple hack for setting a bbox, used for recentering
			minx: 200000000,
			miny: 200000000,
			minz: 200000000,
	...
	OBJLoader.prototype = Object.assign(Object.create(Loader.prototype), {
		...
		parse: function (text) {
				...
				if (lineFirstChar === 'v') {
					...
					switch (data[0]) {
						...
						case 'v':
							...
							// bbox is stored in state object
			...
			container.reference = state.reference;//guardamos las coordenadas de referencia
			...
			if (hasPrimitives === true) {
				...
				for (var i = 0, l = state.objects.length; i < l; i++) {
					...
					///////////////////
					//fix translating object to origin
					console.log('BEFORE: ' + geometry.vertices.length);
					for (let i = 0; i < geometry.vertices.length;) {

						geometry.vertices[i] = geometry.vertices[i] - state.reference.x;
						geometry.vertices[i + 1] = geometry.vertices[i + 1] - state.reference.y;
						geometry.vertices[i + 2] = geometry.vertices[i + 2] - state.reference.z;

						// console.log(geometry.vertices[i] + " " + geometry.vertices[i+1] + " " + geometry.vertices[i+2])
						i = i + 3;
					...
					if (createdMaterials.length > 1) {
						...
						if (isLine) {
							...
							mesh.rotation.set(Math.PI, Math.PI, Math.PI);//segun no hace nada
